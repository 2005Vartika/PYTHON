...
#Probleam - Given a capacity of two jugs j1 and j2, measure given goal (g) amount of water using 8 rules.
            #we can fill jug from pump
            # there is no measuring device
            # these two jugs starting with empty jug 

j1 = int(input("Capacity of jug 1: "))
j2 = int (input("Capacity of jug 2: "))

g = int (input("Amount of water to be measured: "))
goal_jug = int(input("Which jug should contain the goal value? (1 for Jug 1, 2 for Jug 2): "))
...
def apply_rule(ch, x, y):
    ...
    # Rule 3:Transfer all water from jug 1 to jug 2======(0,x+y)
    if ch == 3:
     ...
     # Rule 4:Transfer all water from jug 2 to jug 1=======(x+y,0)
    if ch == 4:
    ...
    # Rule 5:Transfer some water from jug 1 to jug 2 until jug 2 is full =====(x-(j2-y), j2)
    if ch == 5:
    ...
    # Rule 6:Transfer some water from jug 2 to jug 1 until jug 1 is full=====(x-(j1-x), j1)
    if ch == 6:
...
x = y = 0
while(True):
  if (goal_jug == 1 and x == g) or (goal_jug == 2 and y == g):
    print('GOAL ACHIEVED!')
    break
  else:
    print("================================RULES==============================")
    ...
    ch = int(input("Select the rule to be applied: "))
    x, y = apply_rule(ch, x, y)
    print("================================STATUS===========================")
    print(f"CURRENT STATE: Jug1 = {x} liters, Jug2 = {y} liters")
